...
#[derive(Debug, Parser)]
#[command(name = "airbot-play-e2-ws")]
#[command(about = "AIRBOT Play E2 websocket adapter")]
struct Args {
...
mod tests {
    ...
    fn model_backend_flag_parses() {
        let args =
            Args::try_parse_from(["airbot-play-e2-ws", "--model-backend", "play-analytical"])
                .expect("CLI should accept the analytical backend");
        assert_eq!(args.model_backend, ModelBackendKind::PlayAnalytical);
    }

    #[test]
    fn e2_bin_uses_e2_runtime_profile() {
        let args =
            Args::try_parse_from(["airbot-play-e2-ws"]).expect("CLI should use default arguments");
        let config = build_config(args);
        assert_eq!(config.eef_profile, EefRuntimeProfile::E2);
    }
}
